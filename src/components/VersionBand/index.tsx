...
const VersionBand = ({ app }: { app: AppProps }) => {
  ...
  useEffect(() => {
    if (app["active-version"] !== "Local") {
      fetch(
        "https://api.github.com/repos/ameyrupji-com/ameyrupji.com/releases/latest",
      )
        .then((response) => response.json())
        .then((data) => {
          // check if symantic(app.activeVersion) is less than symantic(latestVersion)
          const currentVersion = app["active-version"]
            .replace("v", "")
            .replace(" - Beta", "")
            .split(".")
            .map(Number);
  ...
  return (
    !isLatest && (
      <div
        className="container-full-bg alert alert-warning"
        role="alert"
        id="version-band-main"
      >
        <div className="container">
          Current Version: <span id="version-id">{app["active-version"]}</span>.
          View{" "}
          <a href={app.app_url} className="alert-link">
            latest
          </a>
          .
        </div>
