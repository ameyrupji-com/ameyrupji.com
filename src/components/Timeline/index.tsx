...
const Timeline: React.FC<TimelineProps> = ({ timeline }) => {
  ...
  useEffect(() => {
    setFilteredBlocks(
      timeline.blocks.filter((block) => filters.includes(block.type)),
    );
  }, [filters]);
          ...
          >
            <div className="col-md-12 text-center">
              <h1 className="title">
                ...
                <span className="title-strong">{timeline["title-strong"]}</span>
              </h1>
            </div>
          </div>
            ...
            {filteredBlocks.map((block, index) => {
              if (block.type === "image") {
                return (
                  <div
                    key={index}
                    className={`timeline-block ${block["css-class"].join(" ")}`}
                    id={block["css-id"]}
                  >
                ...
                return <WorkBlock block={block} index={index} key={index} />;
